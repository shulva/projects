...
std::string Found(Course c){
    std::string res = std::format("Found course: {},{},{}",c.title,c.number_of_units,c.quarter);
    return res;
}
...
main(int argc, char* argv[])
{
  ...
  if (argc == 2) {
    ...
    std::string output = course.transform(Found).value_or("Course not found.");

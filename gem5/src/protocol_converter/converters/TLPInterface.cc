...
// 从链路层接收TLP
bool
TLPInterface::receiveTLPFromLinkLayer(PCIe::TLP* tlp)
{
    if (!tlp) {
        return false;
    }
...
// 向链路层发送TLP
bool
TLPInterface::sendTLPToLinkLayer(PCIe::TLP* tlp)
{
    if (!tlp) {
        return false;
    }
...
// 设置TLP处理回调
void
TLPInterface::setTLPCallback(TLPCallback callback)
{
...
// 判断是否为内存读TLP
bool
TLPInterface::isTLPMemoryRead(uint8_t fmt_type)
{
...
// 判断是否为内存写TLP
bool
TLPInterface::isTLPMemoryWrite(uint8_t fmt_type)
{
...
// 判断是否为完成TLP
bool
TLPInterface::isTLPCompletion(uint8_t fmt_type)
{
...
// 判断是否为配置TLP
bool
TLPInterface::isTLPConfiguration(uint8_t fmt_type)
{
...
// 判断是否为消息TLP
bool
TLPInterface::isTLPMessage(uint8_t fmt_type)
{
...
// 获取TLP地址类型
uint8_t
TLPInterface::getTLPAddressType(uint8_t fmt_type)
{
...
// 获取TLP流量类别
uint8_t
TLPInterface::getTLPTrafficClass(uint8_t tc_attr)
{
...
// 获取TLP缓存属性
uint8_t
TLPInterface::getTLPCacheAttr(uint8_t tc_attr)
{
    return tc_attr & 0x0F;
}

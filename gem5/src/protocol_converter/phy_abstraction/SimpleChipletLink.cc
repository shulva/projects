...
// 注册调试标志
Trace::CreateDebugFlag("SimpleChipletLink", "芯粒链路物理层调试信息");
...
PhyProtocolType 
SimpleChipletLink::getProtocolType() const
{
    // 获取协议类型
    return protocolType;
}
...
SimpleChipletLink::sendTLP(void* tlp, uint32_t size, PhyProtocolType protocol)
{
    ...
    // 处理发送数据包
    handleSendPacket(tlp, size, false, protocol);
...
SimpleChipletLink::sendDLLP(void* dllp, uint32_t size, PhyProtocolType protocol)
{
    ...
    // 处理发送数据包
    handleSendPacket(dllp, size, true, protocol);
...
void 
SimpleChipletLink::handleSendPacket(void* data, uint32_t size, bool isDLLP, PhyProtocolType protocol)
{
    // 处理发送数据包
    ...
    // 创建数据包
    PhyPacket* packet = new PhyPacket(data, size, isDLLP, protocol, curTick() + delay);
...
SimpleChipletLink::handleReceiveEvent(PhyPacket* packet)
{
    ...
    if (linkLayerCallback) {
        bool result = linkLayerCallback(packet, protocol, packetType);
...
void 
SimpleChipletLink::handleSendPacket(const void* data, uint32_t size)
{
    // 处理发送数据包
    ...
    // 创建数据包
    PhyPacket* packet = new PhyPacket(data, size, curTick() + delay);
    ...
    DPRINTF(SimpleChipletLink, "调度接收事件: 延迟=%llu ps, 大小=%d 字节\n", 
            delay, size);
}
...
SimpleChipletLink::handleReceiveEvent(PhyPacket* packet)
{
    ...
    // 如果有链路层回调，则直接调用
    if (linkLayerCallback) {
        bool result = linkLayerCallback(packet);
